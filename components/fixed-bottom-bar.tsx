...
const FixedBottomBar = () => {
  ...
  const closeModal = useCallback(() => setIsBookingOpen(false), []);
  ...
  useEffect(() => {
    ...
    if (isBookingOpen) document.addEventListener('keydown', handleKeyDown);
    return () => document.removeEventListener('keydown', handleKeyDown);
  }, [isBookingOpen, closeModal]);
    ...
    <>
      <section className="fixed gap-2 flex justify-center z-40 md:z-50 h-16 shadow-xl bg-white w-full bottom-0 pt-2 pb-6 px-4">
        ...
        >
          ...
          <p className="hidden capitalize md:inline-block text-[#32870f]  font-semibold text-xs" aria-hidden="true">
        ...
        >
          ...
          >
            <div className="flex justify-between items-center p-4 border-b border-zinc-200 sticky top-0 bg-white z-10">
              <h2 id="booking-modal-title" className="text-xl font-semibold text-zinc-900">
            ...
            <div className="relative">
              <iframe
                ...
                title="Book Your Free Test Ride"
                style={{ height: '600px' }}
              />
            </div>
          </div>
